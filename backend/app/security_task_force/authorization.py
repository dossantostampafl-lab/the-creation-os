...
def authorize(
    ...
    *,
    ...
    decision = "permit"
    if (
        action.mission_id != contract.mission_id
        or action.mission_version != contract.mission_version
    ):
        ...
        action.target_id not in contract.authorized_targets
        or action.target_id in contract.excluded_targets
    ):
        ...
        action.risk_class in (RiskClass.R3, RiskClass.R4)
        and not creator_approval_reference
    ):

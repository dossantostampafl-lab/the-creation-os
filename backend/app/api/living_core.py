...
from app.schemas.memory import ConsciousMemoryCreateRequest, ConsciousMemoryResponse, MemoryResponse, MemoryUpsertRequest
...
def inception_response(item) -> InceptionResponse:
    return InceptionResponse(
        ...
        status=item.status,
        trinity_assessment=item.trinity_assessment_json,
...
def memory_response(layer: str, item) -> MemoryResponse:
    ...
    return MemoryResponse(
        id=item.id,
        scope_id=getattr(item, scope_column),
        key=item.key,
...
async def create_inception(
    body: InceptionCreateRequest,
    a: Actor = Depends(actor),
    cid: str = Depends(correlation_id),
    s: LivingCoreService = Depends(service),
):
    return inception_response(
        await s.create_inception(a, body.conversation_id, body.source_message_id, body.title, body.description, cid)
    )
...
async def approve_inception(
    entity_id: uuid.UUID,
    body: InceptionDecisionRequest,
    a: Actor = Depends(actor),
    cid: str = Depends(correlation_id),
    s: LivingCoreService = Depends(service),
):
...
async def list_conscious_memory(
...
async def record_conscious_memory(
    body: ConsciousMemoryCreateRequest,
    a: Actor = Depends(actor),
    cid: str = Depends(correlation_id),
    s: LivingCoreService = Depends(service),
):
...
async def verify_chronicles(a: Actor = Depends(actor), s: LivingCoreService = Depends(service)):
    ...
    return ChronicleVerifyResponse(
        valid=False,
        message=f"{integrity.reason} at event {integrity.first_invalid_event_id}",
    )

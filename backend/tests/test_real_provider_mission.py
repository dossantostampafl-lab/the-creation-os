...
SUPPORTED_REAL_PROVIDERS = {"openai", "freellmapi", "openai_compatible"}


def _configured_provider_and_model() -> tuple[str, str]:
    provider = settings.llm_provider.strip().lower()
    assert provider in SUPPORTED_REAL_PROVIDERS
    if provider == "openai":
        assert settings.llm_api_key is not None and settings.llm_api_key.get_secret_value()
        model = settings.llm_model.strip()
    elif provider == "freellmapi":
        assert os.getenv("FREELLMAPI_API_KEY", "").strip()
        assert os.getenv("FREELLMAPI_BASE_URL", "").strip()
        model = os.getenv("FREELLMAPI_MODEL", "").strip()
    else:
        assert os.getenv("OPENAI_COMPATIBLE_BASE_URL", "").strip()
        model = os.getenv("OPENAI_COMPATIBLE_MODEL", "").strip()
    assert model and model != "fake"
...
async def test_real_provider_completes_authorized_mission_to_manifested(real_provider_db):
    ...
    async with real_provider_db() as session:
        ...
        await service.create_agent(
            ...
            "Real Provider Agent",
            universe.id,
            {"inference_provider": provider, "model": model},
            correlation_id,
        )
    ...
    async with real_provider_db() as session:
        ...
        assert executions[0].provider == provider
